...
class ComputeBinnedLick(object):
    ...
    def compute_lick(self):
        ...
        for i_elem in range(self.cube.nbins):                        
                ...
                if np.isnan(flux_i).all()|(np.nansum(flux_i[~bad_px])<0):
                    continue                                
                ...
                for element in range(len(self.indices)):                    
                    ...
                    # Lick.plot_index(folder='tests/licks/'+self.indices[element]+\
                    #                 '_binreg_'+str(i_elem)+'.png')
                #     break
                # break
    def save_phot_fits(self, path):
        hdr = fits.Header()
...
if __name__=='__main__':        
    cube = CALIFACube(mode='V500', path='PGC11179')
    cube.get_flux()        
